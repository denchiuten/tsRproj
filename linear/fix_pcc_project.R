...
label_id <- "ac0fb724-f338-4d3d-9864-e24d24f36119" #label_id for Core Services label in Platform
project_id <- "5dd4a8f7-46dd-4068-83d5-65c747eefb5b" #project_id for Platform Common Core
  ...
  if(is.null(cursor)) {
    query <- str_glue(
      "{{
        issues(
          filter: {{ 
            attachments: {{url: {{contains: \"{jira_url_base}PCC\"}} }}
            labels: {{every: {{id: {{neq: \"{label_id}\"}} }} }}
          }} 
          first: 100
        ) {{
...
for (i in 1:nrow(df_linear_clean)) {
  ...
  # Check response
  if (is.null(response$errors)) {
    print(str_glue("Assigned {issue_key} to project ({i} of {nrow(df_linear_clean)})"))
  } else {
    print(str_glue("Failed to update issue {issue_key}: Error {response$errors[[1]]$extensions$userPresentableMessage} ({i} of {nrow(df_linear_clean)})"))
  }
}
